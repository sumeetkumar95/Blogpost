...
$table_blogpost_shares = "
CREATE TABLE IF NOT EXISTS BlogPostShares (
    post_id int,
    user_id int,
    CONSTRAINT PK_blogpost_post_id_user_id PRIMARY KEY (post_id, user_id),
    CONSTRAINT FK_post_id_BlogPost_BlogPostShares FOREIGN KEY (post_id) REFERENCES BlogPost (post_id) ON DELETE RESTRICT ON UPDATE RESTRICT,
    CONSTRAINT FK_user_id_User_BlogPostShares FOREIGN KEY (user_id) REFERENCES User (user_id) ON DELETE RESTRICT ON UPDATE RESTRICT
);";
...
$insert_blogpostshares = "
INSERT INTO
BlogPostShares (post_id, user_id)
VALUES (:post_id, :user_id)";
...
$blogs = [
    [
        ...
        "share_count" => 0 // Initial share count
    ],
    [
        "user_id" => 2,
...
try {
    ...
    //------------------------------------------------------
    //   Insert data for blogpost shares
    //------------------------------------------------------
    ...
    foreach($blogs as $blog) {
        ...
        $post_id = $db_connection->lastInsertId(); // Get the ID of the inserted blog post
    
        // Insert initial share count as 0 for each blog post
        $insert_statment = $db_connection->prepare($insert_blogpostshares);
        $insert_statment->bindParam(":post_id", $post_id);
